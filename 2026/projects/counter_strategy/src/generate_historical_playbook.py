...
DEFAULT_PRIORS = {
    ...
    "Runback": {"EV": 1.80, "P(>=3)": 0.250, "P(Steal)": 0.160},
    "Freeze Tap": {"EV": 1.20, "P(>=3)": 0.150, "P(Steal)": 0.080},
    "Takeout": {"EV": 1.80, "P(>=3)": 0.250, "P(Steal)": 0.160}
}
...
def calculate_strategy_metrics(min_samples=15):
    ...
    first_stones = stones[stones["ShotID"] == 1].copy()
    ...
    return pd.DataFrame(strategy_metrics).set_index("Strategy")

def recommend_strategy(scenario, metrics_df):
    profile = scenario["Profile"]
    
    if profile == "Aggressive":
        initial_call = metrics_df["P(Steal)"].idxmax()
    elif profile == "Conservative":
        initial_call = metrics_df["P(>=3)"].idxmin()
    else:
        initial_call = metrics_df["EV"].idxmin()
    
    final_call = initial_call
    if profile == "Conservative" and scenario["End"] >= 6 and initial_call == "Freeze Tap":
        final_call = "Guard Wall"
        
    best_row = metrics_df.loc[final_call]
    return final_call, best_row["EV"], best_row["P(>=3)"], best_row["P(Steal)"]
...
def main():
    ...
    for scenario in SCENARIOS:
        call, ev, p_ge3, p_steal = recommend_strategy(scenario, strategy_metrics_df)
    ...
    playbook_df.to_csv(OUTPUT_DIR / "scenario_playbook_historical.csv", index=False, float_format="%.3f")
    print(f"Successfully generated playbook at {OUTPUT_DIR / 'scenario_playbook_historical.csv'}")
